<?php

use Illuminate\Support\Facades\Route;
use App\Http\Controllers\PegawaiControllers;
use App\Http\Controllers\GajiControllers;

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/pegawai', [PegawaiControllers::class, 'index']);
Route::post('/pegawai', [PegawaiControllers::class, 'store']);

Route::get('/gaji-pegawai', [GajiControllers::class, 'index']);
Route::post('/gaji-pegawai', [GajiControllers::class, 'store']);
Route::post('/gaji-pegawai/batch', [GajiControllers::class, 'batch']);
